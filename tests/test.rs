...
mod with_dsu {
    ...
    fn test_dndtree_matches_idtree_mtx() {
        ...
        for &(u, v) in mtx_data.all_edges.iter() {
            ...
            // for &i in all_nodes.iter() {
            //     for &j in all_nodes.iter() {
            //         let dnd_conn = dnd.query(*i as usize, *j as usize);
            //         let idt_conn = idt.query(*i as usize, *j as usize);
            //         if dnd_conn != idt_conn {
            //             println!(
            //                 "FIRST connectivity mismatch after op on ({u}, {v}) at pair ({}, {}): dnd={}, idt={}",
            //                 i, j, dnd_conn, idt_conn
            //             );
            //             println!("Connectivity diverged");
            //             divergent_state = true;
            //         }
            //     }
            // }
...
mod cpp_tests {
    ...
    // This test will fail if sorting of neighbors is not applied to insert/delete neighbors.
    #[test]
    #[ignore]
    fn test_dndtree_matches_idtree_mtx() {
