...
impl BenchData {
    fn new(filename: &str) -> Self {
        ...
        let dnd_tree = DNDTree::from_adj(&adj_dict);
...
fn trace_creation(data: &BenchData) -> Vec<(i32, usize)> {
    ...
    rc += 1;
    let time = Instant::now();
    let mut tree = DNDTree::new(data.n);
    ...
    rc += 1;
    let time = Instant::now();
    let _tree = DNDTree::from_edges(data.n, &data.edges);
    let elapsed = time.elapsed().as_nanos() as usize;
    trace.push((rc, elapsed));
    ...
    rc += 1;
    let time = Instant::now();
    let _tree = DNDTree::from_adj(&data.adj_dict);
    let elapsed = time.elapsed().as_nanos() as usize;
    trace.push((rc, elapsed));

    // Resetting all edges
    rc += 1;
    let mut tree = DNDTree::from_adj(&data.adj_dict);
    let time = Instant::now();
...
fn trace_query_cold(data: &BenchData) -> Vec<(i32, usize)> {
    let mut tree = data.query_dnd_tree.clone();
...
fn trace_delete(data: &BenchData) -> Vec<(i32, usize)> {
    let mut tree = data.dnd_tree.clone();
    let mut trace = Vec::with_capacity(data.edges.len());
